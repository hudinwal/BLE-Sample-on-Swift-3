...
@objc class BLEAdapter:NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    override init() {
        super.init()
        let controlOptions:[String : AnyObject] = [CBCentralManagerOptionRestoreIdentifierKey:"BLEServiceBrowser" as AnyObject]
    ...
    func connectPeripheral(_ peripheral: CBPeripheral, status: Bool) {
        if(status == true) {
    ...
    func getAllCharacteristicsForService(_ p: CBPeripheral, service: CBService){
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        if (error == nil) {
            ...
            if(self.bleAdapterDelegate != nil){
                self.bleAdapterDelegate?.OnDiscoverService(peripheral.services! as NSArray)
            }
        } else {
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        if ((error == nil)) {
            ...
            if(self.bleAdapterDelegate != nil) {
                self.bleAdapterDelegate?.AllCharacteristics(service.characteristics! as NSArray)
            }
        } else {
    ...
    */
    ...
    func CBUUIDToInt(_ UUID: CBUUID)->UInt16 {
        
        var b1 = [UInt16](repeating: 0, count: 16)
        let byteValues =  [UInt8](UUID.data as Data)
        b1.withUnsafeMutableBytes { destBytes in
    ...
    func CBUUIDToString(_ UUID: CBUUID)->[CChar]?{
        return UUID.data.description.cString(using: String.Encoding.utf8)
    }
    
    func CBUUIDToNSString(_ UUID: CBUUID)->NSString {
        return UUID.data.description as NSString
    }
    
    func UUIDToString(_ UUID: Foundation.UUID?)->[CChar]?{
        if(UUID == nil)
        {
            return  nil
        }
        return UUID!.uuidString.cString(using: String.Encoding.utf8)
    }
    
    func compareCBUUID(_ UUID1: CBUUID, secondUUID  UUID2: CBUUID)->Int {
        var b1 = [UInt8](repeating: 0, count: 16)
        (UUID1.data as Data).copyBytes(to: &b1, count: 16)
        
        var b2 = [UInt8](repeating: 0, count: 16)
        (UUID2.data as Data).copyBytes(to: &b2, count: 16)
    ...
    func compareCBUUIDToInt(_ UUID1: CBUUID, toInt UUID2: UInt16)->Int{
        let uuid1data = UUID1.data
        let uuid1count = uuid1data.count / MemoryLayout<UInt8>.size
        var uuid1array = [UInt8](repeating: 0, count: uuid1count)
        (uuid1data as Data).copyBytes(to: &uuid1array, count: uuid1count * MemoryLayout<UInt8>.size)
